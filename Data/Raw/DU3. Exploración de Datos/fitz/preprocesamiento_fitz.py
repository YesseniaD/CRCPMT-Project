...
pdf_doc = 'LeyGeneraldelEquilibrioEcologicoylaProteccionalAmbiente.pdf'
doc = fitz.open(pdf_doc)

output = open("fitz_text.txt", "wb")
for page in doc:
    txt=page.getText().encode("utf8")
    output.write(txt)
    output.write(b"\n-----\n")
output.close()


#Se hace el conteo de palabras antes de hacer el proceso de eliminaación de ruido
filename = 'fitz_text.txt'
...
file = open('fitz_text.txt', "r", encoding="utf8")
...
try:
    with open(filename2, "r", encoding="utf8") as f_obj:
        contents = f_obj.read()
except FileNotFoundError:
